...
class MatchGame extends Component {
  ...
  getRandomImage = () => {
    ...
    this.setState({activeImage: ImagesList[RandomIndex].imageUrl})
  }

  onClickImage = crId => {
    const {activeImage} = this.state
    const filterdImage = ImagesList.filter(
      each => each.imageUrl === activeImage,
    )
    console.log(filterdImage)
    const {id} = filterdImage[0]
    console.log(id)
    if (crId === id) {
      this.getRandomImage()
  ...
  render() {
    const FilteredImagesList = this.GettingFilteredImages()
    const {activeImage, score, time, isGameOver, activeTabId} = this.state
    console.log(time, isGameOver)
    return (
      <>
        <div>
            ...
            {isGameOver ? (
              ...
              <>
                <div className="image-container">
                  <img src={activeImage} alt="match" className="image-style" />
                </div>
                ...
                <ul className="tabs-container">
                  {TabsList.map(eachTab => (
                    <TabItem
                      ...
                      key={eachTab.tabId}
                      UpdatingActiveTabId={this.UpdatingActiveTabId}
                      isActiveTabId={eachTab.tabId === activeTabId}
                    />
                  ))}
                </ul>
